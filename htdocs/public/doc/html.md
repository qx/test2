...
This makes sure the latest2 version of IE is used in versions of IE that contain
multiple rendering engines. Even if a site visitor is using IE8 or IE9, it's
possible that they're not using the latest2 rendering engine their browser
...
The `meta` tag tells the IE rendering engine it should use the latest2, or edge,
...
the `html` element based on the results of feature test2 and which ensures that
...
Finally, an optimized version of the latest2 Google Analytics tracking code is
